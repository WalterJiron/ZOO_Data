...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ROLLBACK TRANSACTION;
            SET @Mensaje = 'El empleado no esta activo.';
            RETURN;
        END

        SELECT @Mensaje = 
            CASE 
                WHEN EXISTS (SELECT 1 FROM DetalleEmpleado WHERE Cedula = @Cedula )
                    THEN 'La cedula ya esta registrada para otro empleado.'
                WHEN EXISTS (SELECT 1 FROM DetalleEmpleado WHERE INSS = @INSS )
                    THEN 'El INSS ya esta registrado para otro empleado.'
                WHEN EXISTS (SELECT 1 FROM DetalleEmpleado WHERE TelefonoEmergencia = @TelefonoEmergencia)
        ...
        IF EXISTS (SELECT 1 FROM DetalleEmpleado WHERE CodigEmpleado = @CodigoEmpleado)
        BEGIN
            ROLLBACK TRANSACTION;
            SET @Mensaje = 'Ya existe un detalle para el empleado.';
            RETURN;
        END

        INSERT INTO DetalleEmpleado(
            CodigEmpleado,Cedula,
        ...
        BEGIN
            ROLLBACK TRANSACTION;
            SET @Mensaje = 'Error al ingresar el detalle del empleado.';
            RETURN;
        END

        COMMIT TRANSACTION;
        SET @Mensaje = 'Detalle del empleado ingresado correctamente.';
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0
            ROLLBACK TRANSACTION;
